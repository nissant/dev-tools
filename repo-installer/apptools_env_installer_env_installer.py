...
def pre_install(python, repo_dir, requirement_file_list):
    for file in requirement_file_list:
        call_list = [python, '-m', 'pip', 'install', '-r', str(Path(repo_dir) / file)]
        print(call_list)
...
def parse_arguments():
    ...
    parser.add_argument("-u", "--only_reset_and_pull", help="This flag defines if we only fetch.", type=int,
                        default=0)
...
def update_repos(config_file,python, wd, only_reset_and_pull):
    ...
    for item in repo_list:
        ...
        if not repo_data.ignore:
            ...
            if repo_data.pre_install and not only_reset_and_pull:
                pre_install(python, repo_data.repo_dir, repo_data.requirement_file_list)

            #install part
            if repo_data.install and not only_reset_and_pull:
...
if __name__ == '__main__':
    ...
    if not args.only_reset_and_pull:
        upgrade_pip(python)
